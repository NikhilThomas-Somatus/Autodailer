/****** Object:  StoredProcedure [dbo].[sp_AutoDialerCampagin]    Script Date: 7/20/2022 11:57:38 AM ******/
SET ANSI_NULLS ON
GO
...
Declare @PatientActivityId int=0,@IC_DateOfLastCall varchar(max) ='',@IC_CallNotes varchar(max) = '' ,@IC_AgentName varchar(max) ='', @ActivityID int = 0
...
inner join patient b on a.[Member ID] = b.EnrollmentNumber


DELETE FROM ETL_AutoDialer_List_STG
where patientid in (select patientid from patientEligibility where Payername = 'Anthem' and LineofBusinessID = 9542)
...
BEGIN
              ...
              select Top 1 @PatientActivityId=[Patient Activity ID], @IC_DateOfLastCall= case when [IC_DateOfLastCall] is null then '' else cast([IC_DateOfLastCall] as varchar(max)) end, 
             ...
             Update Task 
			Set Comments = Comments+ ' '+@IC_DateOfLastCall+
			...
			modifiedbyuserid = 55
			 ...
			 IF (@ActivityID = 8770 OR  @ActivityID = 8996)
			 	UPDATE TASK SET AssignedToUserID = c.StaffID
				FROM Task a
				inner join ETL_AutoDialer_TMP b on a.patientActivityId = b.[Patient Activity ID]
				inner join RAF_RPT_Scheduler_Crosswalk c on b.IC_Agent_No = c.ContactID
				where a.patientActivityId = @PatientActivityId
...
BEGIN
             ...
             Update Task 
			Set 
            --Comments = Comments+ ' '+@IC_DateOfLastCall+
...
Select * into ETL_AutoDialer_TMP from ETL_AutoDialer_List_STG where [IC_SystemClassification] = '7084'
...
BEGIN
             ...
             Update Task 
			Set Comments = Comments+ ' '+@IC_DateOfLastCall+
			' - 7084 NP Visit + CMA - RING AUTODAILER - '+
			case when @IC_CallNotes is null then '' else @IC_CallNotes end+' - '+case when @IC_AgentName is null then '' else @IC_AgentName end,
			modifiedon = getutcdate(),
			modifiedbyuserid = 55
             where PatientActivityId = @PatientActivityId

             Update PatientActivity  Set ModifiedByUserId=55, ModifiedOn=getutcdate(),ActivityStatusId = 105
             where PatientActivityID = @PatientActivityId
...
TransferToRc = 1,
ModifiedOn = getdate()
where [IC_SystemClassification] = '7084'
...
Select * into ETL_AutoDialer_TMP from ETL_AutoDialer_List_STG where [IC_SystemClassification] = '6513'
...
BEGIN
             ...
             Update Task 
			Set Comments = Comments+ ' '+@IC_DateOfLastCall+
			' - 6513 Caller - Pt. In Hospital/Care Facility - RING AUTODAILER - '+
			case when @IC_CallNotes is null then '' else @IC_CallNotes end+' - '+case when @IC_AgentName is null then '' else @IC_AgentName end,
			modifiedon = getutcdate(),
			modifiedbyuserid = 55
             where PatientActivityId = @PatientActivityId
...
inner join patient b on a.patientid = b.patientid
inner join patientactivity c on a.[patient activity id] = c.patientactivityid
where [IC_SystemClassification] = '6513'
and (c.activityid = 2296 or c.activityid = 2297 or c.activityid = 2298 or c.activityid = 9755 or c.activityid = 9756)
...
Select * into ETL_AutoDialer_TMP from ETL_AutoDialer_List_STG where [IC_SystemClassification] = '6876'
...
BEGIN
             ...
             Update Task 
			Set Comments = Comments+ ' '+@IC_DateOfLastCall+
			' - 6876 Caller - not HIPPA Verified Callback - RING AUTODAILER - '+
			case when @IC_CallNotes is null then '' else @IC_CallNotes end+' - '+case when @IC_AgentName is null then '' else @IC_AgentName end,
			modifiedon = getutcdate(),
			modifiedbyuserid = 55
             where PatientActivityId = @PatientActivityId
...
inner join patient b on a.patientid = b.patientid
inner join patientactivity c on a.[patient activity id] = c.patientactivityid
where [IC_SystemClassification] = '6876'
and (c.activityid = 2296 or c.activityid = 2297 or c.activityid = 2298 or c.activityid = 9755 or c.activityid = 9756)
...
Select * into ETL_AutoDialer_TMP from ETL_AutoDialer_List_STG where [IC_SystemClassification] = '5793'
...
BEGIN
             ...
             Update Task 
			Set Comments = Comments+ ' '+@IC_DateOfLastCall+
			' - 5793 Caller - Left Message w/Person - RING AUTODAILER - '+
			case when @IC_CallNotes is null then '' else @IC_CallNotes end+' - '+case when @IC_AgentName is null then '' else @IC_AgentName end,
			modifiedon = getutcdate(),
			modifiedbyuserid = 55
             where PatientActivityId = @PatientActivityId
...
inner join patient b on a.patientid = b.patientid
inner join patientactivity c on a.[patient activity id] = c.patientactivityid
where [IC_SystemClassification] = '5793'
and (c.activityid = 2296 or c.activityid = 2297 or c.activityid = 2298 or c.activityid = 9755 or c.activityid = 9756)
...
Select * into ETL_AutoDialer_TMP from ETL_AutoDialer_List_STG where [IC_SystemClassification] = '5716'
...
BEGIN
             ...
             Update Task 
			Set Comments = Comments+ ' '+@IC_DateOfLastCall+
			' - 5716 NP Visit - Scheduled - RING AUTODAILER - '+
			case when @IC_CallNotes is null then '' else @IC_CallNotes end+' - '+case when @IC_AgentName is null then '' else @IC_AgentName end,
			modifiedon = getutcdate(),
			modifiedbyuserid = 55
             where PatientActivityId = @PatientActivityId
...
TransferToRc = 1,
ModifiedOn = getdate()
where [IC_SystemClassification] = '5716'
...
Select * into ETL_AutoDialer_TMP from ETL_AutoDialer_List_STG where [IC_SystemClassification] = '5717'
...
BEGIN
             ...
             Update Task 
			Set Comments = Comments+ ' '+@IC_DateOfLastCall+
			' - 5717 NP Visit Decline - Done By Another Company - RING AUTODAILER - '+
			case when @IC_CallNotes is null then '' else @IC_CallNotes end+' - '+case when @IC_AgentName is null then '' else @IC_AgentName end,
			modifiedon = getutcdate(),
			modifiedbyuserid = 55
             where PatientActivityId = @PatientActivityId
...
TransferToRc = 1,
ModifiedOn = getdate()
where [IC_SystemClassification] = '5717'
...
Select * into ETL_AutoDialer_TMP from ETL_AutoDialer_List_STG where [IC_SystemClassification] = '5718'
...
BEGIN
             ...
             Update Task 
			Set Comments = Comments+ ' '+@IC_DateOfLastCall+
			' - 5718 NP Visit Decline - No Tech or No Home Visit - RING AUTODAILER - '+
			case when @IC_CallNotes is null then '' else @IC_CallNotes end+' - '+case when @IC_AgentName is null then '' else @IC_AgentName end,
			modifiedon = getutcdate(),
			modifiedbyuserid = 55
             where PatientActivityId = @PatientActivityId
...
TransferToRc = 1,
ModifiedOn = getdate()
where [IC_SystemClassification] = '5718'
...
Select * into ETL_AutoDialer_TMP from ETL_AutoDialer_List_STG where [IC_SystemClassification] = '6375'
...
BEGIN
             ...
             Update Task 
			Set Comments = Comments+ ' '+@IC_DateOfLastCall+
			' - 6375 NP Visit - Other - RING AUTODAILER - '+
			case when @IC_CallNotes is null then '' else @IC_CallNotes end+' - '+case when @IC_AgentName is null then '' else @IC_AgentName end,
			modifiedon = getutcdate(),
			modifiedbyuserid = 55
             where PatientActivityId = @PatientActivityId
...
TransferToRc = 1,
ModifiedOn = getdate()
where [IC_SystemClassification] = '6375'

COMMIT


------------------------------PROCESS CREATE NEW TASKS--------------------------------------------------
...
select * into SRC_AutoLoaderFrontLoader_Task from SRC_AutoLoaderFrontLoader
...
inner join SRC_AutoLoaderFrontLoader b on a.patientActivityID = b.patientActivityID
...
exec sp_RuleEngineOutReachRuleCheck @Taskid = @taskid, @PatientId=@patientid, @InvokedFrom = 'AutoDialer'
