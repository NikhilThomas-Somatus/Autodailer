/****** Object:  StoredProcedure [dbo].[sp_AutoDialerCampagin]    Script Date: 3/30/2022 12:20:12 AM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
			...
			modifiedon = DATEADD(DAY,-1,DATEADD(DAY,-1,Getutcdate())),
			modifiedbyuserid = 55
             where PatientActivityId = @PatientActivityId

             Update PatientActivity  Set ModifiedByUserId=55, ModifiedOn=DATEADD(DAY,-1,DATEADD(DAY,-1,Getutcdate()))
             where PatientActivityID = @PatientActivityId
...
BEGIN
			...
			modifiedbyuserid = 55
             ...
             Update PatientActivity  Set ModifiedByUserId=55, ModifiedOn=DATEADD(DAY,-1,DATEADD(DAY,-1,Getutcdate())), ActivityStatusId = 249
             where PatientActivityID = @PatientActivityId
...
BEGIN
			...
			case when @IC_CallNotes is null then '' else @IC_CallNotes end+' - '+case when @IC_AgentName is null then '' else @IC_AgentName end,
			modifiedon = DATEADD(DAY,-1,Getutcdate()),
			modifiedbyuserid = 55
             where PatientActivityId = @PatientActivityId

             Update PatientActivity  Set ModifiedByUserId=55, ModifiedOn=DATEADD(DAY,-1,Getutcdate()),ActivityStatusId = 249
             where PatientActivityID = @PatientActivityId
