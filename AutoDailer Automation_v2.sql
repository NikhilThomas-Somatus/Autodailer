...
DROP TABLE if exists #SRC_AutoLoaderFrontLoader


------- Auto Dialer Code 2

drop table  if exists #ETL_AutoDialer_TMP;
...
select patientid 
INTO #SRC_AutoLoaderFrontLoader
from #ETL_AutoDialer_List_STG a
...
DROP TABLE IF EXISTS #ETL_AutoDialer_TMP;
DROP TABLE IF EXISTS #ETL_AutoDialer_OnHoldPatients;
...
select count(1) as rows, a.PatientId 
into E#TL_AutoDialer_OnHoldPatients
from patientcontactinformation a
inner join #ETL_AutoDialer_List_STG b on a.patientid = b.patientid
where [IC_SystemClassification] = '5'
and [IC_AgentDisposition] = '5706'
...
DROP TABLE IF EXISTS  #SRC_AutoDialer_PatientStatus

CREATE TABLE #SRC_AutoDialer_PatientStatus(
...
Begin
	...
	Select top 1 @patient = patientid from SRC_AutoLoaderFrontLoader
...
DELETE FROM #SRC_AutoDialer_PatientStatus
where patientid in (select patientid from patientactivity where activityid = 2297 and activitystatusid = 104)
and patientid in (select patient from patienteligibility where activepatient = 0)
and patientstatus like 'Written Consent%'
and patientstatus like 'On Hold%'
and patientstatus like 'Deceased%'
and patientstatus like 'Verbal Consent%'
and patientstatus like 'Reached%' 


while(select count(*) from #SRC_AutoDialer_PatientStatus) > 0
Begin
...
@patient =  patientid,
@task = c.taskid
from #SRC_AutoDialer_PatientStatus a
inner join #ETL_AutoDialer_List_STG b on a.patientid = b.patient
inner join Task c on b.[patient activity id] = c.patientactivityid


exec sp_RuleEngineOutReachRuleCheck @Taskid = @task, @PatientId=@patient
